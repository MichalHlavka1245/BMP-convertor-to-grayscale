...
bool write_image(string filename, const vector<vector<Pixel>>& image) {
    ...
    set_bytes(bmp_header,  0, 1, 'B');  //identifikátor bmp
    set_bytes(bmp_header,  1, 1, 'M');
    set_bytes(bmp_header,  2, 4, BMP_HEADER_SIZE + DIB_HEADER_SIZE + array_bytes); // velkost suboru
    set_bytes(bmp_header, 10, 4, BMP_HEADER_SIZE + DIB_HEADER_SIZE);  // velkost hlavičky(offset k dátam obrázku)

    set_bytes(dib_header,  0, 4, DIB_HEADER_SIZE);
    set_bytes(dib_header,  4, 4, width_pixels);
    set_bytes(dib_header,  8, 4, height_pixels);
    set_bytes(dib_header, 12, 2, 1); // Počet farebných rovín
    set_bytes(dib_header, 14, 2, 24);  // bitová hlbka -24
    set_bytes(dib_header, 20, 4, array_bytes);  //velkost dat bez hlavicky
    set_bytes(dib_header, 24, 4, 2835); //horizontálne rozlišenie
    set_bytes(dib_header, 28, 4, 2835); //vetikálne rozlišenie
